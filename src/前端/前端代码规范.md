...
1. 项目名
	...
	例 ： my-project-name
2. 目录名 
	全部**以小驼峰命名法**，除第一个单词之外，其他单词首字母大写
	例 ：myDir
3. 文件
	全部采用 小写方式，优先选择 单个单词 命名，多个单词命名以 下划线 _ 分隔
	例：banner_sina.gif、util_helper.js 、my_page.html、my_page.css
4. class/id 
	小写中划线，如header-top
5. 变量函数
...
## HTML 规范
1.  元素 `id` 必须保证页面唯一，同一页面，应避免使用相同的 `name` 与 `id`
